{header}
	...
	<section id="wrapper">
		<!-- Title -->
		<div class="section_title white news_title"><span>تماس با ما</span></div>

		<div class="contact_form">
			<?php if($this->session->flashdata('unsuccess_contact')): ?>
				<div style="color: red"><?= $this->session->flashdata('unsuccess_contact'); ?></div>
			<?php endif; ?>
			<form method="post" action="<?=base_url('contact/send')?>">
				<input type="hidden" name="{csrf_name}" value="{csrf_hash}">
				<input type="text" name="name" placeholder="نام">
				<input type="text" name="email" placeholder="ایمیل (اختیاری)">
				<textarea name="text" placeholder="متن ..."></textarea>
